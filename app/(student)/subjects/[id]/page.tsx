...
export default function StudentSubjectDetailsPage() {
  ...
  useEffect(() => {
    const loadSubject = async () => {
      if (!subjectId) return
      try {
        const response = (await apiFetch(`/student/subjects/${subjectId}`)) as {
          data?: SubjectDetails
        }
  ...
  useEffect(() => {
    const loadTimes = async () => {
      ...
      try {
        const response = (await apiFetch(`/student/timetable?subject_id=${subjectId}`)) as {
          data?: SubjectTime[]
        }
        const data = Array.isArray(response?.data) ? response.data : []
        setSubjectTimes(data.filter((entry) => !entry.subject_id || entry.subject_id === subjectId))
      } catch {
        setSubjectTimes([])
      }
    }
  ...
  useEffect(() => {
    const loadLessons = async () => {
      ...
      try {
        const response = (await apiFetch(`/student/lessons?subject_id=${subjectId}`)) as {
          data?: LessonRow[]
        }
  ...
  const attendanceCount = subject?.attendance_count ?? 0
  const progressPercent =
    totalLessons > 0 ? Math.min(100, Math.round((attendanceCount / totalLessons) * 100)) : 0
  ...
  const chartData = [{ name: "progress", value: progressPercent, fill: "#1D1E1F" }]
                      ...
                      >
                        ...
                        >
                          ...
                          <PolarGrid
                            ...
                            polarRadius={[86, 74]}
                          />
                          <RadialBar dataKey="value" background={{ fill: "#C0C0C0" }} cornerRadius={10} />
                ...
                <div className="overflow-hidden rounded-lg shadow-sm">
                  <div className="relative h-[367px] w-[254px] max-w-full overflow-hidden rounded-lg">
                    {imageUrl ? (
                      ...
                      <img src={imageUrl} alt={subject?.name ?? "subject"} className="h-full w-full object-cover" />
                    ) : (
                        ...
                        <div className="flex flex-1 flex-col gap-6">
                          ...
                          <div className="flex flex-col text-[14px] font-medium leading-[18px] tracking-[-0.01em] text-[#424246]">
                            ...
                            <span className="mt-2 inline-flex w-fit rounded-full px-3 py-1 text-[12px] font-semibold">
                              {lesson.has_media ? (
                                <span className="rounded-full bg-green-100 px-3 py-1 text-green-700">مسجل</span>
                              ) : (
                                <span className="rounded-full bg-red-100 px-3 py-1 text-red-600">غير مسجل</span>
                              )}
                            </span>
                          </div>
                        </div>
